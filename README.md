...
进入游戏后立即后台加载音乐并尝试自动播放；浏览器若限制有声自动播放，第一次点击、触摸或按键操作游戏时自动启动，也可点击“开启音乐”。默认音量为 35%，支持关闭和调节音量，音量偏好保存在本机。手动关闭后，本次页面中的其他操作不会再次启动音乐。切走网页时暂停，回来后尝试恢复。部署时需同时保留根目录的 `music.js`、`music.css` 和 `island-canon-v3.m4a`。
